...
function id_gabung_konsonan($string) {
  ...
  $string = preg_replace(
    "/(B|C|D|F|G|H|J|K|L|M|N|P|Q|R|S|T|V|W|X|Y|Z)\\s?\\1+/",
    "$1",
    $string
  );
  ...
  $string = preg_replace(
    "/(KH|CH|SH|TS|SY|DH|TH|ZH|DZ|GH)\\s?\\1+/",
    "$1",
    $string
  );
...
function id_substitusi_ikhfa($string) {
  ...
  $string = preg_replace(
    "/(A|I|U)NG\\s?(D|F|J|K|P|Q|S|T|V|Z)/",
    "$1N$2",
    $string
  );
...
export { id_fonetik as convert, id_fonetik_tanpavokal as convertNoVowel };
